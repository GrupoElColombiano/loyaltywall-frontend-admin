...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const getListProducts = async (searchText: string, page: number, limit: number, callback: { (result: any): void; (arg0: unknown): void; }): Promise<void> => {
    ...
    let ENDPOINT = `/product/filter-pagination?name=${searchText}&page=${page}&limit=${limit}&idSite=${siteJson.idSite}`;

    if( searchText == ""){
        ENDPOINT = `/product/filter-pagination?name=&page=${page}&limit=${limit}&idSite=${siteJson.idSite}`;
    }
...
export const getSites = (role: string) => {
    return api.get(`/roles/${role}/sites/list`);
};
